...
@base_routes.post('/customer/', response_model=CustomerResponse, status_code=status.HTTP_201_CREATED)
...
async def update_customer(customer_id: int, customer: SingleCustomer, db: db_dependencies):
    db_customer = db.query(models.Customers).filter(models.Customers.customer_id == customer_id).first()
...
@base_routes.post('/order/', response_model=OrderResponse, status_code=status.HTTP_201_CREATED)
...
@base_routes.get('/customers', response_model=List[CustomerResponse], status_code=status.HTTP_200_OK)
async def list_customers(db: db_dependencies):
    customers = db.query(models.Customers).all()
    return customers

@base_routes.get('/customers/{customer_id}/orders', response_model=List[OrderResponse], status_code=status.HTTP_200_OK)
async def list_customer_orders(customer_id: int, db: db_dependencies):
    db.query(models.Customers).filter(models.Customers.customer_id == customer_id).first()
    orders = db.query(models.Orders).filter(models.Orders.customer_id == customer_id).all()
    if not orders:
        raise HTTPException(status_code=404, detail=f'No orders found for customer with id {customer_id}')
    return orders
